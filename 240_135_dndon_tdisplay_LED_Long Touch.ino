/*
  andon_tdisplay.ino  v1.8 (호출: 롱프레스)
...
#include "time.h"
#include "esp_mac.h"
#include "240_135_img.h"   // 대기중 배경 (240x135 RGB565)
...
void loop() {
  ...
  // TTP223: 짧게=해제, 길게(0.8초 이상)=호출
  static bool prevT = false;
  static unsigned long tPress = 0;
  static bool longFired = false;
  bool curT = (digitalRead(TOUCH_PIN) == HIGH);
  if (curT && !prevT) { tPress = now; longFired = false; }        // 눌림 시작
  if (curT && !longFired && now - tPress >= 800) {                // 길게 누름 → 호출
    longFired = true; sendCall();
  }
  if (!curT && prevT) {                                           // 뗌
    if (!longFired && now - tPress >= 40 && msgCount > 0) ackMsgs();  // 짧게 → 해제
  }
  prevT = curT;
